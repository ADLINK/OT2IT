...
extern byte IO_Expander_PortA_Value;
extern byte IO_Expander_PortB_Value;
extern void IO_Expander_Write(byte pin, byte value);
extern byte IO_Expander_Read(byte bank, byte pin);
...
// Adafruit Grand Central M4 GEA (SAMD51)
#elif defined(ADLINK_GEA_M4)

#define TOTAL_ANALOG_PINS       (NUM_ANALOG_INPUTS + NUM_ANALOG_OUTPUTS)
#define TOTAL_PINS              PINS_COUNT
#define VERSION_BLINK_PIN       PIN_LED
#define PIN_SERIAL1_RX          0
#define PIN_SERIAL1_TX          1
#define IS_PIN_DIGITAL(p)       ((p) >=  2 && (p) <= 27)
#define IS_PIN_ANALOG(p)        ((p) >= A0 && (p) <= A8)
#define IS_PIN_PWM(p)           0
#define IS_PIN_SERVO(p)         0
#define IS_PIN_I2C(p)           ((p) == PIN_WIRE_SDA || (p) == PIN_WIRE_SCL)
#define IS_PIN_SPI(p)           ((p) >= MISO && (p) <= SS2)
//#define IS_PIN_SERIAL(p)        ((p) >= 0 && (p) <= 1)
#define IS_PIN_SERIAL(p)        ((p) >= 0 && (p) < 2) || ((p) >= 14 && (p) < 20)
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        ((p) - A0)
#define PIN_TO_PWM(p)           0
#define PIN_TO_SERVO(p)         0

#elif defined(ADAFRUIT_GRAND_CENTRAL_M4) & defined(_SAMD51_) //from CCoE
#define TOTAL_ANALOG_PINS       16 
//#define TOTAL_PINS              96
#define TOTAL_PINS              99
#define IS_PIN_DIGITAL(p)       (((p) >= 0 && (p) <= TOTAL_PINS) && (((p) < 54 || (p) > 54 + TOTAL_ANALOG_PINS)))
#define IS_PIN_ANALOG(p)        ((((p) >= 54 && (p) < 62) || ((p) >= 67 && (p) < 75)))
#define IS_PIN_PWM(p)           (((p) < 2) ? 1 : 0)
#define IS_PIN_SERVO(p)         (IS_PIN_DIGITAL(p) && (p) < MAX_SERVOS) // deprecated since v2.4
#define IS_PIN_I2C(p)           ((p) == 20 || (p) == 21) 
#define IS_PIN_SPI(p)           ((p) == SS || (p) == MOSI || (p) == MISO || (p) == SCK)
//#define IS_PIN_SERIAL(p)        ((p) == 0 || (p) == 1) 
#define IS_PIN_SERIAL(p)        ((p) >= 0 && (p) < 2) || ((p) >= 14 && (p) < 20)
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        ((p >= 67) ? ((p) - 67) : ((p) + 8 - 54))
#define ANALOG_TO_PIN(p)        ((p < 8) ? 67 + (p) : 54 + (p) - 8)
#define PIN_TO_PWM(p)           PIN_TO_DIGITAL(p)
#define PIN_TO_SERVO(p)         (p) // deprecated since v2.4
#undef PIN_SERIAL2_RX
#undef PIN_SERIAL3_RX
#undef PIN_SERIAL4_RX

// Adafruit Grand Central M4 (SAMD51)
#elif defined(ADAFRUIT_GRAND_CENTRAL_M4)
//#define TOTAL_ANALOG_PINS       19//16
#define TOTAL_ANALOG_PINS       16
#define TOTAL_PINS              99//70 // 54 digital + 16 analog
//#define TOTAL_PINS              104
#define VERSION_BLINK_PIN       13
//---------------------------------------------
// It is has been defined in variant.h
//#define PIN_SERIAL1_RX          0//19
//#define PIN_SERIAL1_TX          1//18
//#define PIN_SERIAL2_RX          19
//#define PIN_SERIAL2_TX          18
//#define PIN_SERIAL3_RX          15
//#define PIN_SERIAL3_TX          14
//---------------------------------------------
//#define IS_PIN_DIGITAL(p)       (((p) >=  2 && (p) < 14) || ((p) >= 22 && (p) < 54))
#define IS_PIN_DIGITAL(p)       ((p) < TOTAL_PINS)
#define IS_PIN_ANALOG(p)        (((p) >= 54 && (p) < 62) || ((p) >= 67 && (p) < 75))
#define IS_PIN_PWM(p)           digitalPinHasPWM(p)
#define IS_PIN_SERVO(p)         (IS_PIN_DIGITAL(p) && (p) - 2 < MAX_SERVOS)
#define IS_PIN_I2C(p)           ((p) == PIN_WIRE_SDA || (p) == PIN_WIRE_SCL)//((p) == 20 || (p) == 21)
#define IS_PIN_SPI(p)           ((p) == SS || (p) == MOSI || (p) == MISO || (p) == SCK)
#define IS_PIN_SERIAL(p)        ((p) >= 0 && (p) < 2) || ((p) >= 14 && (p) < 20)
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        ((p) >= 67 ? ((p) - 67) : ((p) - 54 + 8))
#define ANALOG_TO_PIN(p)        ((p < 8) ? 67 + (p) : 54 + (p) - 8)
#define PIN_TO_PWM(p)           PIN_TO_DIGITAL(p)
#define PIN_TO_SERVO(p)         ((p) - 2)
...
static inline unsigned char readPort(byte port, byte bitmask)
{
  ...
  /*if(pin==24){
	if(IO_Expander_Read(0, 0))
	  out |= 0x04;
	if(IO_Expander_Read(0, 1))
	  out |= 0x08;
	if(IO_Expander_Read(0, 2))
	  out |= 0x10;
  }*/
  return out;
#endif
